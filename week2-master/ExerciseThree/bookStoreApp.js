...
function testUser(){
	...
	return {
		balance: balance,
		cart: [],
		deleteBook: function(input) {
			var priceCheck = "These are the books equal to and above "+ input + "$:\n";
			var check = 0;
			for(var i = 0; i < this.cart.length; i++) {
				if(this.cart[i].title === input) {
					confirm("Are sure you want to delete '" + this.cart[i].title + "'?");
					this.cart.splice(i,1);
				}
				else if(this.cart[i].price >= input){
					check++;
					priceCheck += this.cart[i].title + " (" + this.cart[i].price + ")\n";
				} 
			}
			if(check > 0) { 
				var ask = prompt(priceCheck + "Which one would you like to take out of your cart?");
				this.deleteBook(ask);
				alert("'"+ ask + "' has been deleted from your cart!")
			}
		},

		// Clear the Cart
		clearCart: function() {
			confirm("Are SURE you want to clear their cart?");
			this.cart = [];
		},
		checkOut: function() {
			var total = 0;
			for (var i = 0; i < BookStore.length; i++){
				for (var j = 0; j < this.cart.length; j++) {
					if(this.cart[j].title === BookStore[i].title){
						total += this.cart[j].price;
						if (this.balance >= total) {
							alert('You have successfully checked out!')
					}
						else if(this.balance < total) {
							alert('Oops! looks like you do not have enough balance\nPlease upgrade your balance and try again.')
					}
					}
				}
			}
...
// Push the book in Susan's cart if it is the right book
// function checkStore(string) {
// 	for (var i = 0; i < BookStore.length; i++) {
		
// 		if (BookStore[i].title === string){
// 			var ans = prompt("We found your book! " + BookStore[i].title + " by " + BookStore[i].author + " it costs " + BookStore[i].price + "\n" + "Would you like to buy it?");
// 			if (ans === 'yes') {
// 				Susan.cart.push(BookStore[i]);			
// 			} else {
// 				alert("Sorry, your book was not found!");
// 				return;
// 			}  
// 		} 
// 	} 	
...
function checkStore(input) {
	var found = false;
	var list = 0;
	var str = "There are multiple books that are " + input + ", which one would you like?\n";
	for(var i = 0; i < BookStore.length; i++){
		if(BookStore[i].title === input){
			found = true;	
			Susan.cart.push(BookStore[i]);
			prompt("We found your book! " + BookStore[i].title + ", by " + BookStore[i].author + ". It costs " + BookStore[i].price + "\n" + " Would you like to add it to your cart?");
		}
		else if(BookStore[i].category === input) {
			found = true;
			list++;
			str += BookStore[i].title + "\n";
		}	
	}
	if(list > 0) { 
		var ask = prompt(str);
		checkStore(ask);
		alert("'" + ask + "' has been added to your cart!");
	}
	
	if(!found) { 
		alert("The book wasn't found"); 
	}
};

checkStore("Lucky Jim");
checkStore('horror');
checkStore('Tom Jones')
checkStore('Queen Lucia')
checkStore('Catch-22')
console.log(Susan);
Susan.deleteBook(10);
//checkStore("Lucky Jim");
