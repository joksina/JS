			...
			for (var i = 0; i < BookStore.length; i++){
				for (var j = 0; j < this.cart.length; j++) {
					if(this.cart[j].title === BookStore[i].title){
						...
						if (this.balance >= total) {alert('You have successfully checked out!')}
						else if(this.balance < total) {alert('Oops! looks like you do not have enough balance\nPlease upgrade your balance and try again.')}
					}
				}
			}
		}
...
//susan.deleteBook = function(input){}
...
function checkStore(input) {
	...
	if(!found) { alert("The book wasn't found"); }
};
